...
pub(crate) fn split_wasm_file_name(path: &Path, bundle_hash: Option<&str>) -> Result<String> {
    let stem = path
        .file_stem()
        .context("split wasm module is missing a file stem")?
        .to_string_lossy();
    Ok(hashed_file_name(&stem, "wasm", bundle_hash))
}

pub(crate) fn output_file_name(path: impl AsRef<Path>) -> Result<String> {
    Ok(path
        .as_ref()
        .file_name()
...
pub(crate) fn output_file_stem(path: impl AsRef<Path>) -> Result<String> {
    Ok(path
        .as_ref()
        .file_stem()
...
mod tests {
    use super::{
        WasmSplitManifest, hashed_file_name, rewrite_loader_paths, rewrite_prefetch_map,
        split_loader_file_name, split_manifest_file_name,
    };
    ...
    #[test]
    fn rewrites_prefetch_map_with_hashed_filenames() {
        ...
        let rewritten = rewrite_prefetch_map(prefetch_map, &renames).unwrap();
    ...
    #[test]
    fn manifest_serializes_loader_and_prefetch_map() {
