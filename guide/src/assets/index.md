...
- `data-wasm-split`: (optional) enables split-WASM packaging for the main Rust application. This keeps Trunk's standard startup HTML and emits an additional split loader plus auxiliary wasm chunk files. See `examples/wasm-split` and `examples/leptos-split` for end-to-end examples.
